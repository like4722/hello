...
if __name__ == '__main__':
    ...
    root = tk.Tk()
    print("hihi")
    root.title("FFDT快速航线诊断工具")
    root.iconbitmap(r'D:\FFDT\python.ico')
    width, height = root.winfo_reqwidth() + 200, 200  # 窗口大小
    x, y = (root.winfo_screenwidth() - width) / 2, (root.winfo_screenheight() - height) / 2
    root.geometry('%dx%d+%d+%d' % (width, height, x, y))  # 窗口位置居中
    label = tk.Label(root, text="航线中文：", fg="black", font=("黑体", 10))
    label.pack(side=tk.LEFT, expand='no', anchor='nw')
    E1 = tk.Entry(root, bd=3)
    E1.pack(side=tk.LEFT, expand='no', anchor='nw')
    date_str = tk.StringVar()
    date = ttk.Entry(root, textvariable=date_str,state=tk.DISABLED)
    date.place(x=0, y=0, relx=6 / 20, rely=1 / 6, relwidth=6 / 20, relheigh=1 / 5)
    date_str1 = tk.StringVar()
    date1 = ttk.Entry(root, textvariable=date_str1,state=tk.DISABLED)
    date1.place(x=0, y=0, relx=6 / 20, rely=2 / 6, relwidth=6 / 20, relheigh=1 / 5)
    # Calendar((x, y), 'ur').selection() 获取日期，x,y为点坐标
    date_str_gain = lambda: [
        date_str.set(date)
        for date in [Calendar((x, y), 'ur').selection()]
        if date]
    tk.Button(root, text='起始日期:', command=date_str_gain).place(x=0, y=0, relx=1 / 20, rely=1 / 6, relwidth=4 / 20,relheigh=1 / 5)
    date_str1_gain = lambda: [
        date_str1.set(date1)
        for date1 in [Calendar((x, y), 'ur').selection()]
        if date1]
    tk.Button(root, text='结束日期:', command=date_str1_gain).place(x=0, y=0, relx=1 / 20, rely=2 / 6, relwidth=4 / 20,relheigh=1 / 5)
    def submit():
        route = E1.get()
        start_date = date.get()
        end_date = date1.get()
        global argv
        argv = [route, start_date, end_date]
        tkinter.messagebox.showinfo('提示', '请等待，预计需要3分钟！')
        tx.main(argv)
        tkinter.messagebox.showinfo('提示', r'已完成，请至D:\FFDT路径查看报表')
    tk.Button(root, text='确定', command=submit).place(x=0, y=0, relx=1 / 20, rely=4 / 6, relwidth=4 / 20,relheigh=1 / 5)
    tk.Button(root, text='取消', command=root.quit).place(x=0, y=0, relx=6 / 20, rely=4 / 6, relwidth=4 / 20, relheigh=1 / 5)
    root.mainloop()
    endtime1 = datetime.now()
    print("总用时：")
    print((endtime1 - starttime1).microseconds)
...
class App:
    def __init__(self, root):
        ...
        label = tk.Label(root, text="this is a word",bg="pink", fg="red",font=("黑体", 20),width=20,height=10, wraplength=100,justify="left", anchor="center")
        label.pack()
        # 创建一个按钮组件，fg是foreground的缩写，就是设置前景色的意思
        self.hi_there = tk.Button(frame, text="打招呼", fg="blue", command=self.say_hi, width=10, height=2 )
        self.hi_there.pack(side=tk.LEFT)

    def say_hi(self):
        print("互联网的广大朋友们大家好，我是初音未来!")

# 创建一个toplevel的根窗口，并把他作为参数实例化APP对象
#root = tk.Tk()
#app = App(root)
# 开始主事件循环
#root.mainloop()